...
class yuppics_model extends CI_Model{
    ...
    if ($search)
    {
      $dblike = " AND (LOWER(y.title) LIKE '%".strtolower($_GET['search'])."%' OR LOWER(y.author) LIKE '%".strtolower($_GET['search'])."%')";
		...
		$res = $this->db->query("SELECT y.id_yuppic, y.title, y.author, Date(y.created) AS created, yph.background_img AS url_img, yph.bg_pattern
			FROM yuppics AS y
			...
			WHERE y.no_show = 0 AND y.id_customer = ".$this->session->userdata('id_usuario')." AND y.comprado = ".$status.$dblike);
	...
	/**
	 * Elimina por completo un yuppic (para los pendientes)
	 * @param  [type] $id_yuppic [description]
	 * @return [type]            [description]
	 */
	...
	/**
	 * Desactiva un yuppic comprado
	 * @param  [type] $id_yuppic [description]
	 * @return [type]            [description]
	 */
	public function desactivar($id_yuppic){
		$result = $this->db->query("SELECT Count(*) AS cont
		                           FROM yuppics
		                           WHERE id_yuppic = ".$id_yuppic." AND id_customer = ".$this->session->userdata('id_usuario'))->row();
		if($result->cont > 0){
			$this->db->update('yuppics', array('no_show' => '1'), 'id_yuppic = '.$id_yuppic);
			return 1;
		}else
			return 2;
	}
