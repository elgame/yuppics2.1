...
class panel_empleados_model extends privilegios_model{
	...
	public function checkSession(){
		if($this->session->userdata('id_usuario_panel') && $this->session->userdata('nombre_panel') && $this->session->userdata('acceso_panel') === 'admin') {
			if($this->session->userdata('id_usuario_panel')!='' && $this->session->userdata('nombre_panel')!=''){
				return true;
			}
		}
		return false;
	...
	{
		$fun_res = $this->exec_GetWhere('panel_usuarios', $user_data, TRUE);

		if ($fun_res != FALSE)
		{
			$user_data = array(	'id_usuario_panel'=> $fun_res[0]->id_usuario,
													'nombre_panel'  	=> $fun_res[0]->nombre,
													'email_panel'     => $fun_res[0]->email,
                          'acceso_panel'    => 'admin',
													'idunico_panel'		=> uniqid('l', true));
				$this->crea_session($user_data);
		}

			return array($fun_res, 'msg'=>'El email y/o password son incorrectos');
